...
package com.mosoft.optionfusion.ui.widgets.rangebar;
/*
...
public class RangeBar extends View {
    ...
    private static final int DEFAULT_EXPANDED_PIN_WIDTH_DP = 28;
    private static final int DEFAULT_EXPANDED_PIN_HEIGHT_DP = 14;
    ...
    private float mTickHeightPx = DEFAULT_TICK_HEIGHT_DP;
    ...
    private int mPinHeightPx = DEFAULT_EXPANDED_PIN_HEIGHT_DP;
    private int mPinWidthPx = DEFAULT_EXPANDED_PIN_WIDTH_DP;

    private int mTickColor = DEFAULT_TICK_COLOR;

    private float pinScaleDeselected = 1.2f;
    ...
    public Parcelable onSaveInstanceState() {
        ...
        bundle.putFloat("TICK_HEIGHT_DP", mTickHeightPx);
        ...
        bundle.putInt("PIN_HEIGHT", mPinHeightPx);
        bundle.putInt("PIN_WIDTH", mPinWidthPx);
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (state instanceof Bundle) {
            ...
            mTickHeightPx = bundle.getFloat("TICK_HEIGHT_DP");
            ...
            mPinHeightPx = bundle.getInt("PIN_HEIGHT");
            mPinWidthPx = bundle.getInt("PIN_WIDTH");
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mConnectingLine.draw(canvas,
                mLeftThumb == null ? this.getMarginLeft() : mLeftThumb.getX(),
                mRightThumb == null ? this.getWidth() - this.getMarginLeft() : mRightThumb.getX());
    ...
    /**
     * Sets a listener to receive notifications of changes to the RangeBar. This will overwrite any
     * existing set listeners.
     *
     ...
     * Default width = 20dp.
     *
     * @param expandedScale Float specifying the radius of the thumbs to be drawn.
     */
    public void setPinScale(float expandedScale) {
        pinScaleDeselected = expandedScale;
        createPins();
    }
    ...
    /**
     * Does all the functions of the constructor for RangeBar. Called by both RangeBar constructors
     * in lieu of copying the code for each constructor.
     *
    ...
    private void rangeBarInit(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            endedness = Endedness.values()[ta.getInt(R.styleable.RangeBar_endedness, 0)];
            pinScaleDeselected = ta.getFloat(R.styleable.RangeBar_pinScaleMin, 0.6f);
            mTickHeightPx = ta
                    .getDimension(R.styleable.RangeBar_tickHeight, DEFAULT_TICK_HEIGHT_DP);
            mPinHeightPx = (int) ta
                    .getDimension(R.styleable.RangeBar_pinHeight, DEFAULT_EXPANDED_PIN_HEIGHT_DP);
            mPinWidthPx = (int) ta
                    .getDimension(R.styleable.RangeBar_pinWidth, DEFAULT_EXPANDED_PIN_WIDTH_DP);
    ...
    private void createPins() {
        ...
        if (endedness != Endedness.right) {
            mLeftThumb = new PinView(ctx);
            mLeftThumb.init(ctx, yPos, mPinColor, mTextColor, mCircleSize, mCircleColor);
            mLeftThumb.setPinDims(mPinWidthPx, mPinHeightPx);
            mLeftThumb.setScale(pinScaleDeselected);
            mLeftThumb.setPinPadding((int) mPinPadding);
        }

        if (endedness != Endedness.left) {
            mRightThumb = new PinView(ctx);
            mRightThumb.init(ctx, yPos, mPinColor, mTextColor, mCircleSize, mCircleColor);
            mRightThumb.setPinDims(mPinWidthPx, mPinHeightPx);
            mRightThumb.setScale(pinScaleDeselected);
            mRightThumb.setPinPadding((int) mPinPadding);
        }
    ...
    private void onActionDown(float x, float y) {
        ...
        } else if (mRightThumb != null) {
            if (mRightThumb.isInTargetZone(x, y)) {
                pressPin(mRightThumb);
            }
        } else if (mLeftThumb != null) {
    ...
    /**
     * Handles a {@link android.view.MotionEvent#ACTION_UP} or {@link android.view.MotionEvent#ACTION_CANCEL}
     * event.
     *
    ...
    private void onActionUp(float x, float y) {
        ...
        if (mLeftThumb != null && mLeftThumb.isPressed()) {

            releasePin(mLeftThumb);

        } else if (mRightThumb != null && mRightThumb.isPressed()) {

            releasePin(mRightThumb);

        } else {

            float leftThumbXDistance = mLeftThumb != null ? Math.abs(mLeftThumb.getX() - x) : 0;
            float rightThumbXDistance = mRightThumb != null ? Math.abs(mRightThumb.getX() - x) : 0;

            if (leftThumbXDistance < rightThumbXDistance && mLeftThumb != null) {
                if (mLeftThumb != null) {
                    mLeftThumb.setX(x);
                    releasePin(mLeftThumb);
                }
            ...
            final int newLeftIndex = mLeftThumb != null ? mBar.getNearestTickIndex(mLeftThumb) : 0;
            final int newRightIndex = mRightThumb != null ? mBar.getNearestTickIndex(mRightThumb) : 0;
    ...
    private void onActionMove(float x) {
        ...
        if (mLeftThumb != null && mRightThumb != null && mLeftThumb.getX() > mRightThumb.getX()) {
    ...
    /**
     * Set the thumb to be in the pressed state and calls invalidate() to redraw the canvas to
    ...
    private void pressPin(final PinView thumb) {
        ...
        ValueAnimator animator =
                ValueAnimator.ofFloat(pinScaleDeselected, 1)
                        .setDuration(100);
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                thumb.setScale((Float) animation.getAnimatedValue());
                invalidate();
            }
        });
        animator.start();
    ...
    /**
     * Set the thumb to be in the normal/un-pressed state and calls invalidate() to redraw the
     * canvas to reflect the updated state.
     *
    ...
    private void releasePin(final PinView thumb) {
        ...
        ValueAnimator animator = ValueAnimator.ofFloat(1, pinScaleDeselected)
                .setDuration(100);
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
    ...
    /**
     * Set the value on the thumb pin, either from map or calculated from the tick intervals Integer
     * check to format decimals as whole numbers
     *
